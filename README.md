...
Private Pub is a Ruby gem for use with Rails to publish and subscribe to
messages through [Faye](http://faye.jcoglan.com/). It allows you to easily
provide real-time updates through an open socket without tying up a Rails
process. All channels are private so users can only listen to events you
subscribe them to. The original version is authored by Ryan Bates. This is a
fork from that repository to resolve a (hopefully temporary) issue with serving
faye and private\_pub over https
